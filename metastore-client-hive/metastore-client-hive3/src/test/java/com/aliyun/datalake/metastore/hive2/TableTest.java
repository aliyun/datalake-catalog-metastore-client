...
public class TableTest extends BaseTest {
    ...
    public void testDefaultDbCreatTable() throws TException, IOException {
        ...
        ////table doesn't use default db location
        ...
        //1. hive3的内表，使用当前db的location（因为dlf设置的db catalog是空，而不是hive，所以不会使用默认的warehouse location）
        try {
        ...
        //使用当前db的location（因为dlf设置的db catalog是空，而不是hive，所以不会使用默认的warehouse location）
        ...
        //2. 外表，在默认情况下使用默认的external warehouse location
        table.getParameters().put("EXTERNAL", "true");
        HiveConf newConf = new HiveConf();
        newConf.set(ConfVars.WAREHOUSE_EXTERNAL.getVarname(), "file:///tmp/external");
        Warehouse newWh = new Warehouse(newConf);
        IMetaStoreClient newClient = new ProxyMetaStoreClient(newConf);
        try {
            newClient.dropTable(dbName, tblName, true, true, true);
        } catch (Exception e) {
            e.printStackTrace();
        }
        table.getSd().setLocation(null);
        ...
        //在默认情况下使用默认的external warehouse location
        Assert.assertTrue(check.getSd().getLocation().equals(new Path(newWh.getWhRootExternal(), MetaStoreUtils.encodeTableName(table.getTableName().toLowerCase())).toString()));

        //3.外表，设置了使用当前db location为true，那么不会再使用warehouse external
        try {
            client.dropTable(dbName, tblName, true, true, true);
        } catch (Exception e) {
            e.printStackTrace();
        }
        //table location use default db location
        newConf.set(DataLakeConfig.CATALOG_DEFAULT_DB_CREATE_TABLE_USE_CURRENT_DB_LOCATION, "true");
        newClient = new ProxyMetaStoreClient(newConf);
        table.getSd().setLocation(null);
        newClient.createTable(table);
        // check the info about the created table
        check = newClient.getTable(dbName, tblName);
        //设置了使用当前db location为true，那么不会再使用warehouse external
        Assert.assertTrue(check.getSd().getLocation().equals(new Path(new Path(defaultDb.getLocationUri()), MetaStoreUtils.encodeTableName(table.getTableName().toLowerCase())).toString()));
...
//
        ...
        Assert.assertTrue(tableNamesManaged.size() == 2);
...
//
    ...
    public void testAlterTableRenameTable() throws TException {
        ...
        Assert.assertTrue("rename operation gathers stats", checkRenamed.getParameters() == null || checkRenamed.getParameters().get("numFiles") == null);
        Assert.assertTrue("rename operation gathers stats", checkRenamed.getParameters() == null || checkRenamed.getParameters().get("totalSize") == null);
    }
    ...
    @Test
    public void testTableColumnStatisticsWithoutEnableVector() throws TException {
        testTableColumnStatistics(conf, client);
    }

    @Test
    public void testTableColumnStatisticsWithEnableVector() throws TException {
        HiveConf hiveConf = new HiveConf();
        hiveConf.setBoolean("metastore.stats.fetch.bitvector", true);
        testTableColumnStatistics(hiveConf, new ProxyMetaStoreClient(hiveConf));
    }

    public void testTableColumnStatistics(Configuration conf, IMetaStoreClient client) throws TException {
        // create table
        String tblName = TestUtil.TEST_TABLE;
        ...
        for (ColumnStatisticsObj obj : statisticsObjs) {
            if ("id".equals(obj.getColName())) {
                ...
                if (!conf.getBoolean("metastore.stats.fetch.bitvector", false)) {
                    Assert.assertTrue("longBitVectors", getLongColumnStatsData.getBitVectors() == null);
                } else {
                    Assert.assertTrue("longBitVectors", getLongColumnStatsData.bufferForBitVectors().equals(longColumnStatsData.bufferForBitVectors()));
                }
                ...
                if (!conf.getBoolean("metastore.stats.fetch.bitvector", false)) {
                    ...
                    Assert.assertTrue("stringBitVector", getStringColumnStatsData.bufferForBitVectors().equals(stringColumnStatsData.bufferForBitVectors()));
                }
                ...
                if (!conf.getBoolean("metastore.stats.fetch.bitvector", false)) {
                    ...
                    Assert.assertTrue(getDateColumnStatsData.bufferForBitVectors().equals(dateColumnStatsData.bufferForBitVectors()));
                }
                ...
                if (!conf.getBoolean("metastore.stats.fetch.bitvector", false)) {
                    ...
                    Assert.assertTrue("doubleBitVectors", getDoubleColumnStatsData.bufferForBitVectors().equals(doubleColumnStatsData.bufferForBitVectors()));
                }
                ...
                if (!conf.getBoolean("metastore.stats.fetch.bitvector", false)) {
                    ...
                    Assert.assertTrue("decimalBitVectors", getDecimalColumnStatsData.bufferForBitVectors().equals(decimalColumnStatsData.bufferForBitVectors()));
                }
...
//        Assert.assertEquals(LockState.ACQUIRED, res.getState());
    ...
    public void testTruncateTableUnpartitioned() throws Exception {
        ...
        Path dataFile = new Path(table.getSd().getLocation() + "/dataFile");
        client.truncateTable(TEST_DB, tblName, null);
        Assert.assertTrue("Location should exist",
                fs.exists(new Path(table.getSd().getLocation())));
        Assert.assertFalse("DataFile should be removed", fs.exists(dataFile));

    }

    @Test
    public void testTruncateTablePartitioned() throws Exception {
        //不指定分区，即全部分区参与truncate
        testTruncateTablePartitionedCore("null");
    }

    @Test
    public void testTruncateTablePartitionedSpecific() throws Exception {
        //指定部分分区truncate
        testTruncateTablePartitionedCore("part");
    }

    @Test
    public void testTruncateTablePartitionedSpecificExtend() throws Exception {
        //指定所有的分区都需要truncate
        testTruncateTablePartitionedCore("all");
    }

    public void testTruncateTablePartitionedCore(String mode) throws Exception {
        String tblName = TestUtil.TEST_TABLE;
        Map<String, String> columns = ImmutableMap.of("id", "int", "name", "string");
        Map<String, String> partCols = ImmutableMap.of("dt", "string");
        Table tbl = TestUtil.getPartitionTable(TEST_DB, tblName, columns, partCols);
        String[] partValues = {"2020-20-23 12:34:15/1234", "2021-03-03 12:34:15/1234", "2021-03-04 12:34:15/1234"};
        List<String> partitionsToDelete = new ArrayList<String>(){{add(FileUtils.makePartName(Arrays.asList("dt"), Arrays.asList(partValues[0])));add(FileUtils.makePartName(Arrays.asList("dt"), Arrays.asList(partValues[1])));;add(FileUtils.makePartName(Arrays.asList("dt"), Arrays.asList(partValues[2])));}};

        client.createTable(tbl);

        List<Partition> partitions = new ArrayList<>();
        Partition partition = TestUtil.buildPartition(TEST_DB, tblName, tbl, 1, Lists.newArrayList(partValues[0]));
        partitions.add(partition);
        fs.createFile(new Path(partition.getSd().getLocation() + "/datafile"));
        Partition partition2 = TestUtil.buildPartition(TEST_DB, tblName, tbl, 1, Lists.newArrayList(partValues[1]));
        partition2.getSd().setLocation(partition2.getSd().getLocation().replace("/" + TEST_DB, ""));
        partitions.add(partition2);
        fs.createFile(new Path(partition2.getSd().getLocation() + "/datafile"));
        Partition partition3 = TestUtil.buildPartition(TEST_DB, tblName, tbl, 1, Lists.newArrayList(partValues[2]));
        partitions.add(partition3);
        fs.createFile(new Path(partition3.getSd().getLocation() + "/datafile"));
        ...
        if ("null".equalsIgnoreCase(mode)) {
            ...
            partitionsToDelete.remove(2);
            client.truncateTable(TEST_DB, tblName, partitionsToDelete);
        } else {
            client.truncateTable(TEST_DB, tblName, partitionsToDelete);
        }

        Assert.assertTrue("Location should exist",
                fs.exists(new Path(tbl.getSd().getLocation())));
        List<Partition> getPartitions =
                client.listPartitions(TEST_DB, tblName, (short)-1);

        for(Partition part : getPartitions) {
            Path dataFile = new Path(part.getSd().getLocation()+"/datafile");
            if (partitionsToDelete.contains("dt=" + part.getValues().get(0))) {
                Assert.assertFalse("DataFile should be removed:" + dataFile, fs.exists(dataFile));
            } else {
                Assert.assertFalse("DataFile should not be removed:" + dataFile, fs.exists(dataFile));
            }
        }
    }
    ...
    public void testDropTableWhenMetaException() throws Throwable {
        ...
        Mockito.doThrow(DataLakeAPIException.class).when(spy).dropTable(org.mockito.Matchers.any(String.class), org.mockito.Matchers.any(String.class),
                                                                        org.mockito.Matchers.any(String.class), org.mockito.Matchers.any(Boolean.class));
