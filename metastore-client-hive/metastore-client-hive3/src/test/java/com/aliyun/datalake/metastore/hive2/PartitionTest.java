...
public class PartitionTest {
    ...
    public static void initClient() throws TException, FileNotFoundException {
        ...
        databaseName = "metastore_ut_partition_test_from_client";
    ...
    public void testDropPartitions() throws Exception {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            warehouse.mkdirs(new Path(location + "/test1"));
            warehouse.mkdirs(new Path(location + "/test1/test1_1"));
        }
    ...
    public void testTableColumnStatistics() throws TException {
        ...
        client.createTable(table);
        // add partitions
        String partitionName = "DS=20201130A";
        Partition partition = TestUtil.buildPartition(databaseName, tblName, table, 1, Lists.newArrayList("20201130A"));
        Partition addResult = client.add_partition(partition);
        // update statistics
...
//            fail("should have thrown MetaException about partition limit");
    ...
    public void testStatsMerge() throws TException {
        ...
        ColumnStatisticsDesc desc = new ColumnStatisticsDesc(true, databaseName.toUpperCase(), tblName.toUpperCase());
        cs.setStatsDesc(desc);
        ColumnStatisticsObj obj = new ColumnStatisticsObj();
        obj.setColName("a");
        obj.setColType("string");
        ColumnStatisticsData data = new ColumnStatisticsData();
        StringColumnStatsData scsd = new StringColumnStatsDataInspector();
        ...
        StringColumnStatsData getScsd = client.getTableColumnStatistics(databaseName.toUpperCase(), tblName.toUpperCase(), colNames.stream().map(c -> c.toUpperCase()).collect(Collectors.toList())).get(0)
                .getStatsData().getStringStats();
        assertEquals(getScsd.getNumDVs(), 123);

        cs = new ColumnStatistics();
        scsd = new StringColumnStatsDataInspector();
...
//            client.createType(typ1);
    ...
    public void testColumnStatistics() throws Throwable {
        ...
        try {
            ...
            colStats2 = client.getPartitionColumnStatistics(databaseName.toUpperCase(), tblName.toUpperCase(),
                    Lists.newArrayList(partName.replace("ds", "DS")), Lists.newArrayList(colName[1].toUpperCase())).get(partName).get(0);
