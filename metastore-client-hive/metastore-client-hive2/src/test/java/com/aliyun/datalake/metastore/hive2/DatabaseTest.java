...
//        client.dropDatabase("db-2", true, true, true);
    ...
    public void testDropDatabase() throws Exception {
        ...
        // create index table on external table table2
        Index index = TestUtil.buildIndex("index_" + name3, table2);
        Table indexTable = TestUtil.buildIndexTable(index.getIndexTableName(), table2);
        client.createIndex(index, indexTable);
        ValidatorUtils.validateExist(fs, new Path(indexTable.getSd().getLocation()), true);
        ...
        ValidatorUtils.validateExist(fs, new Path(eTable2.getSd().getLocation()), true);
        ValidatorUtils.validateExist(fs, new Path(indexTable.getSd().getLocation()), false);

        fs.delete(new Path(eTable2.getSd().getLocation()), true);

        // check index
        Assert.assertThrows(NoSuchObjectException.class, () -> client.getIndex(name3, table2.getTableName(),
                index.getIndexName()));

        // check table
        Assert.assertThrows(NoSuchObjectException.class, () -> client.getTable(name3, table2.getTableName()));

        // check database tables
        int size = client.getTables(name3, "*").size();
    ...
    public void testGetDatabase() throws Exception {
        ...
        Assert.assertEquals(Lists.newArrayList(db1Name, db2Name), client.getDatabases("metastore_ut_test_db.").parallelStream().filter(d -> (d.equalsIgnoreCase(db1Name) || d.equalsIgnoreCase(db2Name))).collect(Collectors.toList()));
