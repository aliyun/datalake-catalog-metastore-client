...
public class DlfMetaStoreClientDelegate {
    ...
    public DlfMetaStoreClientDelegate(HiveConf conf,
                                      Warehouse warehouse,
        ...
        String partitionValidationRegex =
                this.conf.getVar(HiveConf.ConfVars.METASTORE_PARTITION_NAME_WHITELIST_PATTERN);
        ...
        isAggregateStatsCacheEnabled = HiveConf.getBoolVar(
                this.conf, HiveConf.ConfVars.METASTORE_AGGREGATE_STATS_CACHE_ENABLED);
        if (isAggregateStatsCacheEnabled) {
            aggrStatsCache = AggregateStatsCache.getInstance(this.conf);
        }
        String className = HiveConf.getVar(this.conf, HiveConf.ConfVars.METASTORE_EXPRESSION_PROXY_CLASS);
        try {
            @SuppressWarnings("unchecked")
            Class<? extends PartitionExpressionProxy> clazz =
                    (Class<? extends PartitionExpressionProxy>) MetaStoreUtils.getClass(className);
            expressionProxy = MetaStoreUtils.newInstance(clazz, new Class<?>[0], new Object[0]);
    ...
    @VisibleForTesting
    public void setDataLakeMetaStore(IDataLakeMetaStore dataLakeMetaStore) {
    ...
    public void createDatabase(String catalogId, Database database) throws TException {
        ...
        try {
            ...
            if (!isSuccess && madeDir) {
                hiveShims.deleteDir(warehouse, dbPath, true, enableFsOperation);
            }
        }
    }
        ...
        try {
            ...
            if (!warehouse.isWritable(path)) {
                throw new MetaException("Database not dropped since " +
                        path + " is not writable by " + conf.getUser());
            }

            if (isEmptyDatabase || cascade) {
                try {
                    // delete table which location is not subdirectory of the database location
                    if (deleteData) {
            ...
            if (deleteData) {
                try {
                    hiveShims.deleteDir(warehouse, new Path(dbLocation), true, enableFsOperation);
                } catch (Exception e) {
    ...
    public void dropIndexesForTable(
            String catalogId,
            String dbName,
            Table originTable,
            boolean deleteData
    ) throws TException {
        List<Index> indexesToDelete = getIndicesInTable(originTable);
        for (Index index : indexesToDelete) {
            EnvironmentContext environmentContext = new EnvironmentContext();
            environmentContext.putToProperties("isIndexDelete","true");
            this.dropTable(catalogId, dbName, index.getIndexTableName(), deleteData, true, environmentContext);
        }
    }

    public List<Index> getIndicesInTable(Table originTable) {
        ...
        for (String key : parameters.keySet()) {
            if (key.startsWith(INDEX_PREFIX)) {
                try {
                    ...
                    com.aliyun.datalake20200710.models.Table table = stringToCatalogTable(serialisedString);
                    Index index = CatalogToHiveConverter.ToHiveIndex(table);
                    hiveIndexList.add(index);
                } catch (IOException ignore) {
                    logger.error("failed to covert key to index {}.{}", originTable.getTableName(), key, ignore);
                }
            }
        }
    ...
    public com.aliyun.datalake20200710.models.Table stringToCatalogTable(String s) throws IOException {
        ObjectMapper mapper = new ObjectMapper();
        return mapper.readValue(s, com.aliyun.datalake20200710.models.Table.class);
    }
    ...
    public void createTable(String catalogId, Table tbl, EnvironmentContext envContext) throws TException {
        ...
        boolean isSuccess = false;
        try {
            if (HiveConf.getBoolVar(conf, HiveConf.ConfVars.HIVESTATSAUTOGATHER) && !isView(tbl)) {
            ...
            if (!isSuccess && dirCreated) {
                ...
                hiveShims.deleteDir(this.warehouse, tblPath, true, enableFsOperation);
            }
            if (!isSuccess) {
                try {
    ...
    public List<TableMeta> getTableMeta(String catalogId, String dbPatterns, String tablePatterns, List<String> tableTypes) throws TException {
        ...
        List<Table> tbs = Lists.newArrayList();
    ...
    private void doRename(String catalogId, String dbName, Table oldTable, Table newTable, Database database)
        ...
        destPath = new Path(databasePath, newTable.getTableName().toLowerCase());
        destFs = warehouse.getFs(destPath);
        ...
        try {
            ...
            if (dataWasMoved) {
                try {
                    if (destFs != null && destFs.exists(destPath)) {
            ...
            throw DataLakeUtil.throwException(new MetaException(msg + e.getMessage()), e);
        }
    }
    ...
    private void checkNonViewColumnCompatibility(Table newTable, Table oldTable) throws InvalidOperationException {
        if (HiveConf.getBoolVar(conf, HiveConf.ConfVars.METASTORE_DISALLOW_INCOMPATIBLE_COL_TYPE_CHANGES, false)
                && !isView(oldTable)) {
    ...
    public Path getTablePath(String catalogId, Table table) throws TException {
        ...
        if (StringUtils.isEmpty(table.getSd().getLocation())) {
            ...
            if (db.getName().equalsIgnoreCase("default") && ConfigUtils.getDefaultDbCreateTableUseCurrentDbLocation(conf)) {
                return this.warehouse.getDnsPath(new Path(new Path(db.getLocationUri()), MetaStoreUtils.encodeTableName(table.getTableName().toLowerCase())));
            } else {
                return this.warehouse.getDnsPath(new Path(this.warehouse.getDatabasePath(db), MetaStoreUtils.encodeTableName(table.getTableName().toLowerCase())));
            }
        } else {
    ...
    private void validateTableObject(Table table, HiveConf conf) throws InvalidObjectException {
    ...
    private void validateTableName(Table table, HiveConf conf) throws InvalidObjectException {
    ...
    public void dropTable(String catalogId, String dbName, Table table, boolean deleteData, EnvironmentContext context)
        ...
        //drop indices
        this.dropIndexesForTable(catalogId, dbName, table, shouldDeleteData);

        String tableFullName = dbName + "." + table.getTableName();
        checkTrashPurgeCombination(tablePath, tableFullName, mustPurge, shouldDeleteData);
        ...
        try {
            ...
            if (shouldDeleteData) {
                deletePartitionData(partPaths, mustPurge);
                deleteTableData(tablePath, mustPurge);
            }
        } catch (DataLakeAPIException e) {
            throw CatalogToHiveConverter.toHiveException(e.getResult(), e.getAction(), e);
        } catch (Exception e) {
                        ...
                        if (tablePath == null ||
                            ...
                            if (!warehouse.isWritable(partPath.getParent())) {
                                throw new MetaException("Table metadata not deleted since the partition " +
                                        ...
                                        " which is not writable " + "by " + conf.getUser());
                            }
    ...
    private void checkTrashPurgeCombination(Path pathToData,
        ...
        try {
            trashEnabled = 0 < conf.getFloat("fs.trash.interval", -1);
        ...
        if (trashEnabled) {
            try {
                HadoopShims.HdfsEncryptionShim shim =
                        ShimLoader.getHadoopShims().createHdfsEncryptionShim(FileSystem.get(conf), conf);
    ...
    public void dropTable(
        ...
        validateTableToDrop(table, context);
    ...
    private void deletePartitionData(List<Path> partPaths, boolean ifPurge) {
        if (partPaths != null && !partPaths.isEmpty()) {
            for (Path partPath : partPaths) {
                try {
                    hiveShims.deleteDir(warehouse, partPath, true, ifPurge, false, enableFsOperation);
                } catch (Exception e) {
    ...
    private void deleteTableData(Path tablePath, boolean ifPurge) {
        if (tablePath != null) {
            try {
                hiveShims.deleteDir(warehouse, tablePath, true, ifPurge, false, enableFsOperation);
            } catch (Exception e) {
    ...
    private void validateTableToDrop(Table table, EnvironmentContext context) throws MetaException {
        if (hiveShims.isIndexTable(table)) {
            if (context == null || !"true".equalsIgnoreCase(context.getProperties().get("isIndexDelete"))){
...
//        List<Partition> partitions = getPartitionsByNames(catalogId, columnStatistics.getStatsDesc().getDbName(), columnStatistics.getStatsDesc().getTableName(), partitionNames);
        ...
        try {
            ...
            if (!success) {
                for (String dir : dirsCreated) {
                    hiveShims.deleteDir(warehouse, new Path(dir), true, enableFsOperation);
                }
            }
        }
    }
    ...
    private void initAddedPartition(Table table, Partition partition, boolean madeDir) throws MetaException {
        if (conf.getBoolVar(HiveConf.ConfVars.HIVESTATSAUTOGATHER) && !hiveShims.isView(table)) {
    ...
    private void inheritTableParameters(Table table, Partition partition) {
        ...
        String inheritProps = conf.getVar(HiveConf.ConfVars.METASTORE_PART_INHERIT_TBL_PROPS).trim();
        ...
        try {
            ...
            if (shouldDelete) {
                ...
                for (Path path : archToDelete) {
                    hiveShims.deleteDir(warehouse, path, true, mustPurge, false, enableFsOperation);
                }
                for (PathAndPartValSize p : dirsToDelete) {
                    hiveShims.deleteDir(warehouse, p.getPath(), true, mustPurge, false, enableFsOperation);
                    try {
                        deleteParentRecursive(p.getPath().getParent(), p.getPartValSize() - 1, mustPurge);
                    } catch (IOException ex) {
            ...
            throw DataLakeUtil.throwException(new MetaException(message + e), e);
        }
    }
    ...
    private void verifyPathRemovable(
        ...
        checkTrashPurgeCombination(path, objectName, mustPurge, shouldDelete);
    }
    ...
    private void deleteParentRecursive(Path parent, int depth, boolean mustPurge) throws IOException, MetaException {
        if (depth > 0 && parent != null && warehouse.isWritable(parent)) {
            if (warehouse.isDir(parent) && Utils.isEmptyDir(warehouse, parent)) {
                hiveShims.deleteDir(warehouse, parent, true, mustPurge, false, enableFsOperation);
            }
    ...
    public List<PartitionSpec> listPartitionSpecs(
        ...
        try {
            Table table = getTable(cateName, dbName, tblName);
    ...
    public AggrStats getAggrColStatsFor(
        ...
        if (isAggregateStatsCacheEnabled
            ...
            for (String colName : lowerCaseColNames) {
                ...
                colStatsAggrCached = aggrStatsCache.get(dbNameLowercase, tblNameLowercase, colName, lowerCasePartNames);
                if (colStatsAggrCached != null) {
                    ...
                    if (!colStatsAggrFromDB.isEmpty()) {
                        ...
                        aggrStatsCache.add(dbNameLowercase, tblNameLowercase, colName, partsFound, colStatsAggr, bloomFilter);
                    }
                }
            }
            return new AggrStats(colStatsList, partsFound);
        } else {
            AggrStats aggrStats = commonMetaStoreClientDelegate.getAggrStatsFor(catalogId, req);
            return aggrStats;
        }
    }
    ...
    private boolean useInternalSerde(Table tbl) {
        ...
        Collection<String> internalSerdes = conf.getStringCollection(
                HiveConf.ConfVars.SERDESUSINGMETASTOREFORSCHEMA.varname);
    ...
    public void alterFunction(
        ...
        try {
            ...
            try {
                ...
                throw DataLakeUtil.throwException(new MetaException(e1.getMessage()), e1);
            }
        } catch (Exception e) {
